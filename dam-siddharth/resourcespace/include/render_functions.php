...
function render_featured_collections(array $ctx, array $items)
    ...
    foreach($items as $fc)
        ...
        // Add Template option - only show for collections with NO children
        if($is_featured_collection)
            {
            $child_count = ps_value("SELECT COUNT(*) as value FROM collection WHERE parent = ?", ["i", $fc['ref']], 0);
            if ($child_count == 0 && collection_writeable($fc['ref'])) {
                $render_ctx["tools"][] = array(
                    "href" => "#",
                    "icon" => 'fa fa-fw fa-folder-plus',
                    "text" => 'Add Template',
                    'custom_onclick' => sprintf("showTemplateConfirmation(%s, '%s'); return false;", escape($fc['ref']), escape($fc['name'])),
                );
            }
            }
        ...
        if ($is_featured_collection_category && !$is_smart_featured_collection) {
            ...
            // Delete option - always show for featured collection categories
            if (
                can_delete_collection(get_collection($fc['ref']), $userref ?? 0, $k ?? '')
                && can_delete_featured_collection($fc['ref'])
            ) {
                $render_ctx["tools"][] = array(
                    "href"            => "#",
            ...
            // Download folder (all resources recursively)
            $render_ctx["tools"][] = array(
                "href" => generateURL(
                    "{$baseurl_short}pages/ajax/download_collection_tree.php",
                    ['collection_ref' => $fc['ref']]
                ),
                "icon"           => 'fa-solid fa-fw fa-download',
                "text"           => $lang["zipall"] ?? 'Download',
                "custom_onclick" => sprintf(
                    "window.location.href='%s'; return false;",
                    escape(generateURL(
                        "{$baseurl_short}pages/ajax/download_collection_tree.php",
                        ['collection_ref' => $fc['ref']]
                    ))
            ...
            $render_ctx["tools"][] = array(
                ...
                "text"           => 'Copy folder',
                "custom_onclick" => sprintf(
                    "rsCopyFolder(%s, '%s', '%s'); return false;",
                    escape($fc['ref']),
                    escape(strip_prefix_chars($fc['name'], '*')),
                    escape(generate_csrf_js_object('copy_collection_tree'))
                ),
            );

            // Paste Here option - only show when something is in clipboard
            $render_ctx["tools"][] = array(
                "href"           => "#",
                "icon"           => 'fa-solid fa-fw fa-paste',
                "text"           => 'Paste here',
                "custom_onclick" => sprintf(
                    "rsPasteFolder(%s, '%s'); return false;",
                    escape($fc['ref']),
                    escape(generate_csrf_js_object('copy_collection_tree'))
                ),
            );
            
            // Add Template option for featured collection categories - only if NO children
            $child_count = ps_value("SELECT COUNT(*) as value FROM collection WHERE parent = ?", ["i", $fc['ref']], 0);
            if ($child_count == 0) {
                $render_ctx["tools"][] = array(
                    "href" => "#",
