...
function search_filter($search, $archive, $restypes, $recent_search_daylimit, $access_override, $return_disk_usage, $editable_only = false, $access = null, $smartsearch = false)
{
    ...
    if (!$access_override) {
        ...
        if (!checkperm("v") && !(substr($search, 0, 11) == "!collection" && ($k != '' && $collection_allow_not_approved_share || count($archive) > 0))) {
            ...
            // NOTE: Collection searches bypass this filter when explicit archive states are provided
            // (e.g., allow_multi_edit function) or when sharing with external access key
            $rtexclusions = "";
            $rtexclusions_params = [];
            for ($n = 0; $n < count($userpermissions); $n++) {
