...
// Reading the collection tables and showing the collection count
$collection_count = ps_value("SELECT COUNT(*) value FROM collection", array(), 0);
echo "<!-- Total collections in database: " . $collection_count . " -->";
...
if ('' != $name && can_create_collections() && enforcePostRequest(false)) {
    // Reading the collection tables and showing the collection count
    $total_collections = ps_value("SELECT COUNT(*) value FROM collection", array(), 0);
    echo "Total collections: " . $total_collections . "<br>";
    
    // Get latest/max ID from collection table
    $latest_id = ps_value("SELECT MAX(ref) value FROM collection", array(), 0);
    echo "Latest collection ID: " . $latest_id . "<br>";
    
    // Calculate next ID
    $next_id = $latest_id + 1;
    echo "Next ID will be: " . $next_id . "<br>";
    
    // Append ID to collection name
    $name = $name . " (ID-" . $next_id . ")";
    echo "Collection name with ID: " . $name . "<br>";
    
    // Create new collection
    $new = create_collection($userref, $name);
    echo "Created collection with actual ID: " . $new . "<br>";
    ...
    if ($enable_themes && getval("call_to_action_tile", "") === "true" && checkperm("h")) {
        ...
        $redirect_params["reload"] = "true"; // Force cache clear on redirect
