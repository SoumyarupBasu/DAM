    ...
    if (isset($error)) {
        ...
        <form method=post id="collectionform" action="<?php echo $form_action; ?>" onsubmit="return <?php echo $modal ? 'Modal' : 'CentralSpace'; ?>Post(this, false);">
            ...
            <div class="Question">
                ...
                <?php
                // Split name and ID suffix e.g. "Prakhar (ID-3000)" -> "Prakhar" + " (ID-3000)"
                $full_name = $collection["name"];
                $id_suffix = '';
                $base_name = $full_name;
                if (preg_match('/^(.*?)(\s*\(ID-\d+\))$/', $full_name, $matches)) {
                    $base_name  = $matches[1];
                    $id_suffix  = $matches[2];
                }
                ?>
                ...
                <input
                    ...
                    class="stdwidth"
                    id="name_display"
                    value="<?php echo escape($base_name) ?>"
                    maxlength="100"
                    <?php if ($collection["cant_delete"] == 1) { ?>
                        readonly=true
                    <?php } else { ?>
                        oninput="document.getElementById('name_full').value = this.value<?php if ($id_suffix) { ?> + '<?php echo escape($id_suffix); ?>'<?php } ?>;"
                    <?php } ?>
                >
                <?php if ($id_suffix) { ?>
                    <span style="white-space:nowrap;color:#aaa;font-size:13px;"><?php echo escape($id_suffix); ?></span>
                <?php } ?>
                </div>
