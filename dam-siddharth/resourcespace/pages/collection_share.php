...
elseif (is_featured_collection_category($collection)) {
    // Allow sharing of empty FC categories - removed restriction
...
// Check if collection is empty (used for multiple checks below)
$collection_resources = get_collection_resources($ref);
$is_empty_collection = empty($collection_resources);
...
// Skip not approved check for empty collections
if (!$collection_allow_not_approved_share && !$is_empty_collection && !$collectionstates) {
        $show_error = true;
...
if (!$restricted_share && !$is_empty_collection && ($minaccess >= RESOURCE_ACCESS_RESTRICTED || $resource_count != count(do_search("!collection{$ref}", '', 'relevance', 0, -1, 'desc', false, '', false, '', '', false, false)))) {
    $show_error = true;
