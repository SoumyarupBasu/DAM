...
$folder_upload = false; // Track if this is a folder (featured collection) upload mode
...
if ($upload_review_mode)
    ...
    if($external_upload !== false) 
        ...
        // Check if we're uploading to a featured collection folder
        $folder_upload = false;
        if(is_int_loose($collection_add) && $collection_add > 0)
            {
            $folder_check = ps_query("SELECT ref, type FROM collection WHERE ref = ? LIMIT 1", ['i', $collection_add]);
            if(is_array($folder_check) && count($folder_check) > 0 && $folder_check[0]['type'] == 3)
                {
                // Destination folder for uploaded files; review queue stays in the user's temp collection
                $folder_upload = true;
                debug("Folder upload mode enabled - destination folder: $collection_add");
                }
            }
        }

    # Make sure review collection is clear of any resources moved out of users archive status permissions by other users
    if (!$edit_access_for_contributor && !$folder_upload)
        {
    ...
    $usersearchfilter = "";
    $review_collection_contents = do_search("!collection{$collection}", '', 'resourceid', 0, -1, 'DESC');
            ...
            if ($ref > 0 && getval("save","") != "" && enforcePostRequest($ajax))
                ...
                if ($upload_review_mode)
                    ...
                    if($external_upload)
                        ...
                        {
                        // For folder uploads, resource should already be in the destination folder
                        // Only remove from user temporary collection if NOT a folder upload
                        if(!$folder_upload)
                            {
                        ...
                        // If uploading to a folder (featured collection), ensure resource is in folder
                        if($folder_upload && is_int_loose($collection_add) && $collection_add > 0)
                            {
                            // First remove from user collection if it's there
                            remove_resource_from_collection($ref,0-$userref);
                            // Then ensure it's in the folder
                            debug("Ensuring resource $ref is in folder $collection_add");
                            add_resource_to_collection($ref, $collection_add, false, "", $resource['resource_type']);
                            }
                        }
                    ...
                    if($save_auto_next)
                        ...
                        for($n=1;$n<$review_collection_contents_count;$n++)
                            ...
                            // If no errors, remove from review collection and ensure file is in destination folder
...
<div class="BasicsBoxRight">
    ...
    $original_ref = $ref;
    // Do not pull a related resource's image during upload review — always show the actual uploaded file
    if (!$upload_review_mode && (int)$resource['has_image'] === RESOURCE_PREVIEWS_NONE) {
