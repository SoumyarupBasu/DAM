...
# AUTO VERSION INCREMENT PLUGIN: Include version restore function
if (file_exists(__DIR__ . '/../../auto_version_increment/hooks/rse_version_revert.php'))
    {
    include_once __DIR__ . '/../../auto_version_increment/hooks/rse_version_revert.php';
    }
...
elseif($type==LOG_CODE_UPLOADED)
    ...
    if (getval("revert_action","")=="revert" && enforcePostRequest(false))
        ...
        if($revertok===true)
            {
            # AUTO VERSION INCREMENT PLUGIN: Restore version number
            if (function_exists('auto_version_restore_on_revert'))
                {
                auto_version_restore_on_revert($resource, $ref);
                }
