...
function add_resource_to_collection(
    ...
    // Featured collections may hold both subfolders and files at the same level (Google Drive-style).
    // Do not block adds solely because the folder already has child collections.
    $collection_is_featured = (ps_value(
        "SELECT type AS value FROM collection WHERE ref = ?",
        ["i", $collection],
        0
...
function create_collection($userid, $name, $allowchanges = 0, $cant_delete = 0, $ref = 0, $public = false, $extraparams = array())
{
    ...
    // PLUGIN: Append collection ID to name immediately
    if ($ref > 0 && !preg_match('/\s*\(ID-\d+\)$/', $name)) {
        $name_with_id = mb_strcut($name, 0, 100) . " (ID-" . $ref . ")";
        ps_query("UPDATE collection SET name = ? WHERE ref = ?", array("s", $name_with_id, "i", $ref));
        
        // Clear ALL collection-related caches aggressively
        // Reset the "already completed" tracker to force cache clearing
        global $query_cache_already_completed_this_time;
        if (isset($query_cache_already_completed_this_time)) {
            $query_cache_already_completed_this_time = array_diff(
                $query_cache_already_completed_this_time, 
                array("collection", "collection" . $ref, "featured_collections")
            );
        }
        
        // Clear query caches
        clear_query_cache("collection");
        clear_query_cache("collection" . $ref);
        clear_query_cache("featured_collections");
        
        // Clear the global get_collection cache
        if (isset($GLOBALS['get_collection_cache'])) {
            if (isset($GLOBALS['get_collection_cache'][$ref])) {
                unset($GLOBALS['get_collection_cache'][$ref]);
            }
            // Clear all collection caches to be safe
            $GLOBALS['get_collection_cache'] = array();
        }
        
        // Clear featured collections permission cache
        if (isset($GLOBALS['CACHE_FC_PERMS_FILTER_SQL'])) {
            $GLOBALS['CACHE_FC_PERMS_FILTER_SQL'] = array();
        }
        
        error_log("Collection ID Plugin: Appended ID to collection $ref: $name -> $name_with_id");
    }
...
function delete_collection($collection)
{
    ...
    $children = ps_array("SELECT ref AS value FROM collection WHERE parent = ?", array("i", $ref));
    foreach ($children as $child_ref) {
        delete_collection($child_ref);
    }
...
function allow_multi_edit($collection, $collectionid = 0)
{
    ...
    if (is_array($collection) && $collectionid == 0) {
        ...
        # Include appropriate search states for contributors to see their own uploads
        $collection_search_states = "0,-2,-1"; // Active, Pending submission, Pending review
        ...
        if (!is_array($collection)) {
            ...
            $collection = do_search("!collection{$collectionid}", '', '', $collection_search_states, -1, '', false, 0, false, false, '', false, false, true, false);
        } else {
            # When collection is passed as array, collectionid should be provided as second parameter
            if ($collectionid == 0) {
                # This is an edge case - try to get collection ID from collection data if available
                return true; # Default to allowing multi-edit for resource arrays without collection context
            }
        }
        if (is_array($collection)) {
        ...
        $editresults =  do_search("!collection{$collectionid}", '', '', $collection_search_states, -1, '', false, 0, false, false, '', false, false, true, true);
...
function new_featured_collection_form(int $parent)
{
    ...
    <div class="BasicsBox" id="create_new">
        ...
        <form name="new_collection_form" id="new_collection_form" class="modalform"
            ...
            <input type="hidden" name="call_to_action_tile" value="true">
            <input type="hidden" name="parent" value="<?php echo $parent; ?>">
            <div class="Question">
                <label for="newcollection"><?php echo escape($lang["collectionname"]); ?></label>
                <input type="text" name="name" id="newcollection" maxlength="100" required="true" class="medwidth">
                <div class="clearerleft"></div>
            </div>
            <div class="Question">
                <input type="submit" name="create" value="<?php echo escape($lang["save"]); ?>" class="medcomplementwidth" />
                <input type="button" value="<?php echo escape($lang["cancel"]); ?>" onclick="ModalClose();" class="medcomplementwidth" />
                <div class="clearerleft"></div>
            </div>
        </form>
    </div>
...
function is_featured_collection_category(array $fc)
{
    ...
    // A collection is a category if it has subfolders (children), regardless of whether it also has resources
    // This allows Google Drive-style co-existence of folders and files at the same level
    if ($fc["type"] == COLLECTION_TYPE_FEATURED && !empty($fc["has_children"])) {
        return true;
    }

    if (!isset($fc["has_resources"])) {
        return false;
    }
...
{
    // Allow deletion of any featured collection, including those with children or resources
    // Children and resources will be recursively deleted
    $sql = "SELECT ref AS `value` FROM collection WHERE `type` = ? AND ref = ?";
...
function allow_upload_to_collection(array $c)
{
    ...
    if (in_array($c["type"], [COLLECTION_TYPE_SELECTION, COLLECTION_TYPE_REQUEST])) {
        return false;
    }
