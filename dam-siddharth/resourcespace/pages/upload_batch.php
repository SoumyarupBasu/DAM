...
$target_folder_id = getval('target_folder_id', 0, true); // For files uploaded to folders
$upload_to_folders = getval('upload_to_folders', 0, true); // Flag that we're uploading to folders
$from_folder_upload = (bool) getval('from_folder_upload', false); // Skip review collection for folder uploads

// Handle folder upload mode - retrieve folderMap from localStorage (passed via JavaScript)
$folder_map = array();
if($upload_to_folders)
    {
    // In folder upload mode, collection_add is the primary folder
    // Files will be routed to their specific folders based on their paths
    debug("Folder upload mode enabled - collection_add: $collection_add");
    }
...
elseif ($upload_then_edit && $replace == "" && $replace_resource == "")
    ...
    $post_upload_redirecturl = $redirecturl;
    if (is_numeric($collection_add)) {
        $collection_add_data = get_collection((int)$collection_add);
        if ($collection_add_data && (int)$collection_add_data['type'] === COLLECTION_TYPE_FEATURED) {
            // Return to the folder view after metadata entry (files appear beside subfolders)
            $post_upload_redirecturl = generateURL($baseurl . '/pages/collections_featured.php', array('parent' => $collection_add));
        } elseif ($upload_here) {
            // Standard collection — redirect to collection search view
            $post_upload_redirecturl = generateURL($baseurl . '/pages/search.php', array('search' => '!collection' . $collection_add));
        }
    }
    ...
    $redirecturl = generateURL(
        ...
        array(
            ...
            'redirecturl' => $post_upload_redirecturl,
        ));
...
if ($processupload)
    {
    // Route each file to its target subfolder when uploading from a folder structure
    $file_target_folder_id = getval('target_folder_id', 0, true);
    debug("Folder upload: file_target_folder_id from request = $file_target_folder_id");
    debug("Folder upload: collection_add before override = $collection_add");
    if ($file_target_folder_id > 0)
        {
        $collection_add = $file_target_folder_id;
        debug("Folder upload: routing file to collection $collection_add");
        }
    else
        {
        debug("Folder upload: no target_folder_id, using collection_add = $collection_add");
        }
        ...
        elseif ($replace=="" && $replace_resource=="")
            ...
            if(!$ref)
                ...
                if ($upload_then_edit && $replace == "" && $replace_resource == "" && $collection_add != $upload_review_col)
                    {
                    # Also add to the user's special upload collection for metadata review.
...
// Handle folder upload: retrieve folder map and route files accordingly
var folderUploadMap = {};
var folderUploadPaths = [];
if (typeof(Storage) !== 'undefined') {
    var storedMap = window.localStorage.getItem('folderUploadMap');
    var storedPaths = window.localStorage.getItem('folderUploadPaths');
    if (storedMap) {
        try {
            folderUploadMap = JSON.parse(storedMap);
            folderUploadPaths = storedPaths ? JSON.parse(storedPaths) : [];
            console.log('Folder upload mode detected. Folder map:', folderUploadMap);
        } catch(e) {
            console.error('Failed to parse folder upload data:', e);
        }
    }
}
...
jQuery(document).ready(function () {
    ...
    jQuery('#folderUploadButton').on('click', function() {
        var parentRef = '<?php echo (int)$collection_add; ?>';
        if (typeof rsShowCreateChoice === 'function') {
            // Use the existing folder upload popup
            rsShowCreateChoice('<?php echo addslashes($baseurl) ?>/pages/collections.php?new=true', parentRef);
        }
    });
    ...
    uppy = new Uppy.Uppy({
        ...
        onBeforeUpload: (files) => {
            ...
            console.log(files[fileid]);
                ...
                var uploadDisplayName = files[fileid].meta.original_filename || files[fileid].name;
                parts = uploadDisplayName.split('.');
                extension = parts.pop();
...
<?php
    ...
    <?php if ($upload_to_folders) { ?>
    // Folder upload mode — queue all files from the selected local folder into Uppy
    if (typeof rsQueueFolderUploadFiles === 'function') {
        rsQueueFolderUploadFiles(0);
    }
    <?php } ?>
...
function processFile(file, forcepost)
    ...
    var originalName = (file.meta && file.meta.original_filename) ? file.meta.original_filename : file.name;
    console.debug("Processing file: " + originalName);

    // Determine target folder from relative path at any nesting depth
    var targetFolderId = 0;
    var relativePath = file.webkitRelativePath || (file.meta && file.meta.relative_path) || file.name || '';
    relativePath = relativePath.replace(/\\/g, '/');

    if (file.meta && file.meta.target_folder_id) {
        targetFolderId = parseInt(file.meta.target_folder_id, 10) || 0;
    }

    if (!targetFolderId && Object.keys(folderUploadMap).length > 0 && relativePath) {
        var rootCol = (typeof newcol !== 'undefined') ? (parseInt(newcol, 10) || 0) : 0;
        if (typeof rsResolveFolderUploadTarget === 'function') {
            targetFolderId = rsResolveFolderUploadTarget(relativePath, folderUploadMap, rootCol);
        } else {
            var pathParts = relativePath.split('/');
            if (pathParts.length > 1) {
                var folderPath = pathParts.slice(0, -1).join('/');
                if (folderUploadMap.hasOwnProperty(folderPath)) {
                    targetFolderId = folderUploadMap[folderPath];
                }
            }
        }
        if (targetFolderId) {
            console.log('File ' + originalName + ' routed to folder ' + targetFolderId + ' (path: ' + relativePath + ')');
        }
    }
    ...
    postdata = {
        ...
        key: file.meta.key ?? "",
        target_folder_id: targetFolderId || (file.meta.target_folder_id ?? 0),
        from_folder_upload: (targetFolderId > 0) || (file.meta.from_folder_upload ?? false),
        <?php if($CSRF_enabled)
            {
...
<div class="BasicsBox">
    <!-- Folder upload option button -->
    <div id="folderUploadOption" style="margin-bottom: 20px; padding: 15px; background: #f5f5f5; border-radius: 6px; text-align: center; <?php echo ($collection_add == 'false' || $collection_add == '' || !is_numeric($collection_add)) ? 'display: none;' : ''; ?>">
        <button id="folderUploadButton" type="button" style="background: #3a8c3f; color: #fff; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500; margin-right: 10px;">
            <i class="fa-solid fa-folder-arrow-up" style="margin-right: 8px;"></i>Upload Folder
        </button>
        <span style="color: #666; font-size: 13px; margin-left: 10px;">or</span>
        <span style="color: #666; font-size: 13px; margin-left: 10px;">Upload individual files below</span>
    </div>
        <div id="uploader" ></div>
</div>
<?php
